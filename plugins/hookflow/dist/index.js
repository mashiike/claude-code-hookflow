...
function defaultStatePathResolver(event) {
...
function removeState(event, resolver) {
  ...
  try {
    ...
    if (err instanceof Error && "code" in err && (err.code === "ENOENT" || err.code === "ENOTEMPTY")) {
      return;
    }
...
// src/workflow.ts
var DEFAULT_EVENT_NAMES = ["Stop", "TaskCompleted"];
...
function parseWorkflowFile(filePath) {
      ...
      for (const rawStep of def.steps) {
        if (rawStep && typeof rawStep.run === "string") {
          steps.push({
            ...
            stop_reason: typeof rawStep.stop_reason === "string" ? rawStep.stop_reason : void 0
          });
        }
      }
      if (steps.length > 0) {
        jobs[key] = {
          ...
          stop_reason: typeof def.stop_reason === "string" ? def.stop_reason : void 0
        };
      }
    }
  }
  return {
    ...
    stop_reason: typeof raw.stop_reason === "string" ? raw.stop_reason : void 0,
    on,
    paths,
...
function listYamlFiles(dir) {
  try {
    ...
    return entries.filter((f) => /\.(ya?ml)$/.test(f)).map((f) => path3.join(dir, f)).sort();
  } catch {
    return [];
  }
}
function loadWorkflows(cwd) {
  const workflows = [];
  const seen = /* @__PURE__ */ new Set();
  const projectDir = path3.join(cwd, ".claude", "hookflows");
  for (const file of listYamlFiles(projectDir)) {
    const w = parseWorkflowFile(file);
    if (w) {
      workflows.push(w);
      seen.add(path3.basename(file));
    }
  }
  const globalDir = path3.join(os.homedir(), ".claude", "hookflows");
  for (const file of listYamlFiles(globalDir)) {
    if (seen.has(path3.basename(file))) {
      continue;
    }
    const w = parseWorkflowFile(file);
    if (w) {
...
function matchWorkflow(workflow, changedFiles, cwd, trigger) {
  ...
  for (const file of changedFiles) {
    if (path3.isAbsolute(file) && !workflow.external_files) {
      continue;
    }
    const relative3 = path3.isAbsolute(file) ? path3.relative(cwd, file) : file;
    const included = workflow.paths.some((pat) => minimatch(relative3, pat));
    if (!included) continue;
    const excluded = workflow.paths_ignore.some((pat) => minimatch(relative3, pat));
    if (excluded) continue;
    if (!matched.includes(relative3)) {
      matched.push(relative3);
    }
  }
  return matched;
}
...
var App = class {
  ...
  handleUserPromptSubmit(event) {
    ...
    state.cwd = event.cwd;
    const prompt = event._raw.prompt;
    state.changed_files = [];
